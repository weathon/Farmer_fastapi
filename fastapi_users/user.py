...
from fastapi_users.password import get_password_hash, get_password_pin
...
def get_create_user(
    ...
    async def create_user(
        ...
        # Added by Weathon
        hashed_pin=get_password_pin(user.pin)
        # Added by weathon
        user_dict = (
            user.create_update_dict() if safe else user.create_update_dict_superuser()
        )
        # print(hashed_pin)
        # print(user_dict) #pin 包含在里面
        db_user = user_db_model(**user_dict, hashed_password=hashed_password, hashed_pin=hashed_pin)
        # print(db_user)
